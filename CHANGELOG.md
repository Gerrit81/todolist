...
## v3.2.2 (2026-08-27)
### 修复 — 预排实例标记完成后循环任务在月历中消失（整体系列被误终结）
- 🐛 **根因**：`due_datetime` 被误填成循环终止日期（如创建任务时把「截止日期」填成 2030-12-31，而循环终止日期也设为 2030-12-31）。此时在月历中勾选 27 号预排实例，`completeTask` 基于异常 due 计算下一期（2031-01-27）超出终止日期 → 走「普通完成」分支标记 `is_completed=1` → 循环任务整体系列被终结，后续所有月份 27 号全部消失。
- 🔧 **修复一（核心）**：`completeTask()` 新增可选参数 `$dueDate`（当前完成期次日期）。前端预排/虚拟实例勾选时携带其 `due_date`，后端以该期次为基准计算下一期并推进，不再受异常 `due_datetime` 影响。
  - `toggle_task` / `update_task` / `update_task_status` 三个接口均透传 `due_date`
- 🛡️ **修复二（兜底防御）**：当 `next` 超出 `recurrence_end` 且当前 due 本身已超出终止日期（异常数据）时，从 `recurrence_start`/`created_at` 重新定位终止日期内最后一个期次并推进，避免误终结系列。
- 🎨 **前端适配**：`toggleTask(id, s, dueDate)` 支持第三参数；预排实例 checkbox 的 `onclick` 传入 `t.due_date`
