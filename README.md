...
项目内置了一套**零配置**的数据库备份系统：

**触发方式（任选其一）：**

| 方式 | 说明 |
|------|------|
| **惰性触发（默认）** | 每次 API 请求时自动检查，当天未备份则自动执行。无需任何配置 |
| **Cron 定时任务** | `0 3 * * * php /path/to/backup.php` — 每天凌晨 3 点执行 |
| **HTTP 令牌调用** | 在 `backup.php` 中设置 `HTTP_TOKEN`，通过 `GET /backup.php?token=xxx` 远程触发 |

**备份策略：**
- 使用 SQLite 原生 `backup()` API，保证数据一致性（WAL → checkpoint → backup）
- 备份文件存放于 `data/backups/`，命名格式：`todolist_backup_YYYY-MM-DD_HHmmss.db`
- 自动清理旧备份，最多保留 **15 份**（在 `config.php` 的 `backup_max` 中调整）
- 同一天多次触发只产生一份备份（按日期去重）

**恢复备份：**
```bash
# 停止 Web 服务后，将备份文件覆盖回原位置
cp data/backups/todolist_backup_2026-07-18_030000.db data/todolist.db
```
...
├── config.php         # 配置 / 数据库 / 认证 / SMTP / 加密 / 备份
├── backup.php         # 数据库备份脚本（v2.2.8+）
...
从 v1.0.0 到 v2.4.1，横跨 20+ 个版本的关键节点一览：
...
| **v2.2.8** | 数据库自动备份（15 份轮转）、SQLite 原生 backup API、cron/惰性/HTTP 三种触发 |
| **v2.2.9** | 修复月/年重复任务首次出现计算偏移、逆向计算同步修正 |
| **v2.3.0** | 「保持登录」复选框、Session 30 天持久化、防会话固定攻击 |
| **v2.3.1** | 修复 Cookie 覆盖致登录丢失、周/月/年循环首次偏移、表单字段顺序 |
| **v2.3.2** | 修复重复任务「🔁 X次」标志覆盖已完成复选框 |
| **v2.3.3** | 修复循环标志溢出覆盖操作按钮、侧栏循环任务计数为 0 |
| **v2.3.4** | 修复虚拟重复实例的 🔁 图标覆盖复选框问题 |
| **v2.3.5** | 修复虚拟实例 ID 丢失致点击无响应/编辑报 400 |
| **v2.4.0** | 登录页毛玻璃风格 + 主题选择弹窗 + 3 套护眼新主题 + 修复 IIS 自动登出 |
| **v2.4.1** | 新增毛玻璃主题 frost + 修复主题按钮点击无响应 |
